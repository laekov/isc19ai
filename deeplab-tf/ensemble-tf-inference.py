...
from tiramisu_model import create_tiramisu
...
def main(device, input_path_test, downsampling_fact, downsampling_mode, channels, data_format, label_id, weights, image_dir, checkpoint_dir, output_graph_file, tst_sz, loss_type, model, decoder, fs_type, batch, batchnorm, dtype, scale_factor, predmode):
    ...
    with test_graph.as_default():
        ...
        with tf.variable_scope('model0'):
            model0 = deeplab_v3_plus_generator(num_classes=3, output_stride=8,
                                              ...
                                              scope_prefix='model0/')
            logit0, prediction0 = model0(next_elem[0], True, dtype)
        with tf.variable_scope('model1'):
            model1 = deeplab_v3_plus_generator(num_classes=3, output_stride=8,
                                              ...
                                              data_format=data_format,
                                              scope_prefix='model1/')
            logit1, prediction1 = model1(next_elem[0], True, dtype)

        with tf.variable_scope('modelt'):
            # Harded coded here
            blocks = [3, 3, 4, 4, 7, 7]
            growth = 32
            nb_filter = 64
            ...
            ne0t = tf.transpose(next_elem[0], [0, 3, 1, 2])
            logitt, predictiont = create_tiramisu(3, ne0t, image_height, image_width, \
        ...
        logit = logit0 + logit1 + logitt * 2.5
        prediction = prediction0 + prediction1 + predictiont * 2.5
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            modelvars = dict([(v.name[7:-2], v) for v in tf.global_variables(scope='model0')])
            load_model(sess, tf.train.Saver(var_list=modelvars, name='model0/'), checkpoint_dir + '_model0')

            modelvars = dict([(v.name[7:-2], v) for v in tf.global_variables(scope='model1')])
            load_model(sess, tf.train.Saver(var_list=modelvars, name='model1/'), checkpoint_dir + '_model1')

            modelvars = dict([(v.name[7:-2], v) for v in tf.global_variables(scope='modelt')])
            load_model(sess, tf.train.Saver(var_list=modelvars, name='modelt/'), checkpoint_dir + '_modelt')
            ...
            while True:
                try:
                    ...
                    #print some images
                    if have_imsave and not predmode:
